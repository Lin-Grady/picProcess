var picProcess = {
         ...
         */
        selectFileImage: function(e, selector) {
                ...
                fReader.onload = function(e) {
                        ...
                        img.onload = function() {
                                ...
                                var url = picProcess.changeBase64.call(this, orientation);
         ...
         * @return {[type]}             [Base64]
         */
        changeBase64: function(orientation) {
                var canvas = document.createElement('canvas'),
                        ctx = canvas.getContext('2d'),
                        nSize = picProcess.resetSize(this, 800, 900);

                canvas.width = nSize.width;
                canvas.height = nSize.height;
                ctx.drawImage(this, 0, 0, nSize.width, nSize.height);

                console.log('navigator信息: ', navigator);
                if (navigator.userAgent.match(/Android/i)) {
                        var encoder = new JEPGEncoder();
                        return encoder.encode(ctx.getImageData(0, 0, nSize.width, nSize.height), 80)
                } else {
                        if (orientation && orientation != 1) {
                                rotateImg(this, canvas, orientation);
                        }
                        return canvas.toDataURL('image/jepg', 0.8);
                }
        },

        /**
         * [重設尺寸]
         * @param {[type]} img   [base64編碼]
         * @param {[type]} ratio [比例]
         */
        resetSize: function(img, width, height, ratio, max) {
                var w = img.naturalWidth,
                        ...
                        width = width || w,
                        height = height || h;
                console.log('圖片高寬: ', w, h);

                if(w != width) w = width;
                if(h != height) h = height;
                if(ratio && w/h != ratio) h = ratio*h;
                //限制最大值範圍
                if(max){
                        if (w > max && w > h) {
                                h = Math.ceil(h * max / w);
